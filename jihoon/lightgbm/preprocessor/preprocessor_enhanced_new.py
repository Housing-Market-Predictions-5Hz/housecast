# 개선된 전처리 코드 (기존 기능 + 유재성님 참고 Feature 추가)
...
def map_nearest_bdong(df, bdong_df):
    if {'좌표X', '좌표Y'}.issubset(df.columns):
        coords = df[['좌표Y', '좌표X']].to_numpy()
        nbrs = NearestNeighbors(n_neighbors=1).fit(bdong_df[['위도', '경도']])
        distances, indices = nbrs.kneighbors(coords)
        df['법정동'] = bdong_df.iloc[indices.flatten()]['동'].values
        df['법정동코드'] = bdong_df.iloc[indices.flatten()]['법정동코드'].values
    return df
...
def preprocess_enhanced(train, test, bus, subway, radius_km=1.0):
    ...
    # 좌표 기반 법정동 매핑
    bdong_df = load_bdong()
    combined = map_nearest_bdong(combined, bdong_df)
    ...
    if "전용면적(㎡)" in combined.columns:
        ...
        combined["log_area"] = np.log1p(combined["전용면적(㎡)"])
    ...
    if "target" in combined.columns:
        ...
        combined["면적당_가격"] = combined["target"] / (combined["전용면적(㎡)"] + 1)
